...
onBloodGlucoseReading → { mac, type, value, unit, dataID, timestamp, canCorrect, source }  // Android: one per stored record, then onGlucoseMeterEvent stage "offline_synced". `timestamp` is the METER's clock (UTC, parsed as GMT); canCorrect=true means taken before it was set (SDK DATA_TIME_PROOF — same meaning as iOS canCorrect, NOT "time is proven"). iOS pulls records via debugBG5SGetOfflineData instead (each has measureDate + canCorrect).
